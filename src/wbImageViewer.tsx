...
export const Loading: React.FC<{ className?: string | undefined }> = (props) => {
  return (
...
export const LoadingImg = forwardRef<HTMLImageElement, LoadingImgProps>(function LoadingImgSource(props, ref) {
...
export function WbImageViewer(props: WbImageViewerProps) {
  ...
  const wrapDom = useRef<HTMLDivElement | null>(null)
  const tg = useRef<TouchGesture>()

  useEffect(() => {
    if (src) {
      tg.current && tg.current.destroy()
      tg.current = new TouchGesture((imgDom.current as HTMLImageElement))
      const t2d = new Transform2D((imgDom.current as HTMLImageElement))
  ...
  return (
    <div
      className={'wbv-wrap ' + (visibility ? 'visible' : 'un-visible')} style={{display: visibleStyle}}
      ref={wrapDom}
      onAnimationEnd={() => {
        if (!visibility) {
